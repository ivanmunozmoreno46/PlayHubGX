...
| 2026-04-21 | `Pulido` | TBD | **Eliminado el multiplayer legacy**: se retira el botón `MP: ON/OFF` y el panel `MultiplayerLobby` de la barra superior de `EmulatorScreen`. Se borran los ficheros `src/hooks/useMultiplayer.js` y `src/components/MultiplayerLobby.jsx` (la feature estaba siendo sustituida de hecho por Game Room, que ofrece streaming + input forwarding en lugar de chat + lista de slots). El paquete final queda más pequeño (~11 kB menos en el bundle JS gzip). |
