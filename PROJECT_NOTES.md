...
| 2026-04-21 | `Pulido` | #4 | **Carcasa PS1 más fiel al SCPH-1001**. Se rediseña el shell para leerse como una PS1 original: en el top face un bloque de plástico gris con a) el logo `PS` a dos letras superpuestas (P amarillo/naranja + S roja, con sombra 3D), b) el wordmark `PlayStation` italic silkscreen, c) un disc lid circular grande con `ps1-disc-lid` (gradiente radial, surco concéntrico e impresión interna) y d) un botón negro `OPEN` fijado en la esquina superior derecha del lid. La franja frontal pasa a contener los slots reales: `MEMORY CARD 1/2` (ranuras oscuras con pines serigrafiados), `CONTROLLER 1/2` (puertos con la forma característica redondeada arriba y pines dorados) y a la derecha un `POWER` rectangular rojo (clase `.ps1-power-switch` con estado `is-on` que ilumina) y un `RESET` rectangular oscuro (`.ps1-button-dark`). En el medio se mantiene una rejilla de ventilación `ps1-vents`. Los colores y relieves son puro CSS (sin imágenes). |
| 2026-04-21 | `Pulido` | #4 | **Carcasa PS1 gris hardware (iteración inicial)**. Se cambia la estética del contenedor exterior para que imite la consola SCPH-1001: plástico gris claro con relieves (`.ps1-shell`, `.ps1-shell-dark`, `.ps1-well`, `.ps1-vents`, `.ps1-plastic-texture`, `.ps1-silkscreen` en `src/index.css`), rejillas de ventilación, pozo oscuro con la "pantalla" del emulador, y barra frontal con tres botones físicos: POWER (rojo), RESET (negro), OPEN (gris). Nuevos tokens de paleta `ps1-plastic*`, `ps1-inner*`, `ps1-power-red*`, `ps1-reset-black`, `ps1-open-gray`. `App.jsx` reescrito como carcasa con slots de Memory Card y Controller silkscreen en la franja inferior. Los tokens `ps1-bios-*` vuelven a apuntar a azul navy porque representan la imagen mostrada en la "TV" dentro del pozo (EmulatorScreen, GameRoomPanel, GuestControlsPanel siguen usándolos sin cambios). |
...
| 2026-04-21 | `Pulido` | #4 | **Rediseño total a escritorio BIOS "Memory Card Manager" (Option C evolucionado)**. Se elimina la carcasa física PS1 (plástico, puertos, discos, POWER/RESET) y la página entera pasa a ser un escritorio BIOS a pantalla completa imitando el menú original del SCPH-1001. Nuevo componente `src/components/BiosDesktop.jsx`: fondo de cuadrícula de "save blocks" (12×8) con los primeros bloques iluminados en verde/amarillo cuando BIOS/GAME están cargados, dos paneles `MEMORY CARD SLOT 1/2` en la cabecera (número 1 verde, número 2 amarillo, sobre bloque granate oscuro), panel central de acciones con botones tostado/dorado (LOAD BIOS · LOAD GAME · START GAME) con texto granate y relieve PS1, tira LCD debajo con el nombre del archivo cargado y fila inferior de pills confirm-style (RESET rojo, ROOM azul). Se añaden clases `.ps1-bios-slot-panel`, `.ps1-bios-action` y `.ps1-bios-pill` en `src/index.css` para los relieves, hover y active states. `App.jsx` se simplifica a un wrapper `h-screen overflow-hidden` que monta directamente `<EmulatorScreen />` (fuera el `isPoweredOn` y todo el shell físico). `EmulatorScreen.jsx` sustituye el loader unificado anterior por un único `<BiosDesktop />` conectado a `triggerBiosInput`, `triggerRomInput`, `startEmulator`, `resetEmulator` y al toggle de Game Room; se conservan íntegras las pantallas de loading, running (canvas) y error, así como la barra superior (PAD/ROOM), los puertos inferiores y la vista de streaming del invitado. Funcionalidad intacta (useEmulator, useGameRoom, useGamepad, file inputs, shortcuts). |
| 2026-04-29 | `turn-cloudflare` | — | **Cloudflare TURN endpoint (relay-only)**. Añadido servidor proxy sin servidor de Vercel `/api/turn-credentials` para la gestión segura de las claves e infraestructura TURN en tiempo real. Ahora `getPeerRtcConfig()` solicita claves efímeras para garantizar conexiones P2P estables, el fichero `turn.js` incluye su lógica y sistema de caché, y el `turn-cloudflare` se sincroniza totalmente. También se eliminó definitivamente el legacy obsoleto de PartyKit. |
